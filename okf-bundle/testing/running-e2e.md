...
| Next-Gen Android backend selection/stub or `tooling/appium/test/specs/nextgen.fail-fast.spec.ts` | Build with `yarn tests:android:build:nextgen`, then run the durable named `yarn tests:appium:android:nextgen-smoke`. Apply other matching rows independently. |
