...
The pure calculator supports slots `0`–`7`. For slot `N` and platform offset `P`, `BASE = 12000 + 1000N + P`, where Android `P=0`, iOS `P=100`, and macOS `P=200`; raw platform resources use Appium `BASE+13`, automation `BASE+14`, and MJPEG `BASE+15`, while the Android console is `5556+2N` (`emulator-<console>`). Operational Metro is worktree-scoped and never platform-offset: `12000 + 1000N + 7` for both Android and iOS. Android AVDs are the cross-repository names `TestingAVD-N`; iOS simulators are `RN E2E iOS slot-N`. Every shared Android slot must use an API 36 Play Store-enabled system image: Native ads return SDK internal error 0 on otherwise healthy `google_apis` images even while Banner and fullscreen formats load, which previously allowed false-green Native evidence. The macOS offset is reserved for compatible cross-repository arithmetic; RNGMA has no macOS-app e2e target.
...
The command reuses an available exact existing name and otherwise creates it. It never deletes, erases, renames, overwrites, or repairs any device—even when an existing exact-name AVD has an incompatible ABI or system image. For a missing Android AVD, local slot provisioning installs the API 36 `google_apis_playstore` image matching the Node host (`arm64-v8a` on `arm64`, `x86_64` on `x64`) and creates the shared `TestingAVD-N`; any other host architecture is rejected before inventory or mutation. Rebuilding that name with a plain `google_apis` image invalidates Native fill/render coverage without making other ad formats fail, so operators must verify the Play Store target when rebuilding shared slots. iOS creates an exact iPhone 17 on the newest available installed iOS runtime. Provisioning requires a slot and rejects `0` and `3`. A single-platform argument must match `RNGMA_E2E_PLATFORM` when that variable is set; `both` requires it unset. Those rejections happen before inventory or create. Appium and `ios:select-and-boot` remain select-and-boot only: they never create a device. A missing exact device is a blocker until this provisioning command succeeds.
...
Set `RNGMA_E2E_PARALLEL_SLOTS` to exactly three comma-separated operational slots when the defaults are unsuitable. Pairing is positional: first/second/third map to `a-primary`, `b-secondary`, and `c-tertiary`; their test counts are derived, not configured ([§ Appium](#appium-scaffold)). The product default remains `1,2,4`; for a host where those slots are not all owned, select only owned slots, for example:

```bash
RNGMA_E2E_PARALLEL_SLOTS=1,4,5 yarn tests:appium:android:parallel
```

The aggregate is the derived suite split across the three sessions, not three full copies of it. Parsing rejects a count other than three, empty/malformed values, duplicates, slot `0`, reserved slot `3`, out-of-range slots, and conflicting slot/platform/port child environment. Do not set internal spec-filter, parent/child, or Metro-slot variables and do not invoke the workspace implementation directly. Existing serial commands and the manual per-slot sequence above are unchanged.
...
Each invocation creates a new `/tmp/rngma-e2e/<invocation-id>/` tree; it never truncates evidence from an earlier invocation. The tree holds `metro.log`, one platform/slot/label child log per Appium stream, and exact-device startup logs. Parallel summaries print the invocation ID and log root, and each result row retains slot/spec/tests/status/numeric exit/log. Startup failure and cleanup semantics are canonical in [§ enforced startup supervision](#enforced-startup-supervision). A successful run prints one passing row per positional shard and the derived total from `tooling/appium/src/sessionShards.ts` ([§ Appium](#appium-scaffold)), then an owner parent stops its one Metro; an external parent never does. Release interrupted slots individually; release the shared Metro only from its explicit owner slot scope.
...
<a id="no-source-edits-during-a-run"></a>

**No source edits during a run.** Before editing product/harness sources: are e2e tests running in your slots? If yes, wait until they complete (or stop them); if no, proceed with edits, then start a fresh run. Metro hot refresh otherwise corrupts in-flight sessions — procedure in [change authoring](change-authoring-workflow.md#no-edits-during-live-e2e).
...
Private workspace `@invertase/rngma-appium` at `tooling/appium/` (Yarn workspace `tooling/*`; not a Lerna publish package). Stack: Appium 3 + WebdriverIO + UiAutomator2 + XCUITest. Broad **navigation/container smoke** samples representative **Banner** + **Collapsible Banner**, listed GAM banner sizes (**AnchoredAdaptiveBanner**, **Fluid**), plus interstitial / rewarded / rewarded interstitial / app open / native / GAM interstitial, and hooks / inspector / debug seams; the consent gallery row is navigation-only ([§ API-wide e2e coverage](#api-wide-e2e-coverage)). Navigation smoke does not claim those ads loaded. Remaining banner size variants stay reachable via the gallery accordion (manual QA).

<a id="api-wide-e2e-coverage"></a>

### API-wide e2e coverage

The smoke suite is gated by checked-in registries, not ad-hoc format lists alone. Together they answer, for every runtime export from `packages/core/src/index.ts`, whether Appium must prove an outcome, why not, and which native product files a flushed device run should touch.

**Public API matrix (`tooling/appium/src/publicApiMatrix.ts`).** `derivePublicApiMatrix()` loads the core package barrel with `packages/core/tsconfig.json` and classifies each export (`class`, `function`, `hook`, `component`, `const/preset`, `namespace/object`, or `type-only`). Namespace/object exports enumerate runtime members (`AdPools.create`, `AdsConsent.gatherConsent`, …). Explicit type-only re-exports need no hand-written registry row; everything with a runtime value must be dispositioned.

**Contracts registry (`tooling/appium/src/contracts.ts`).** `PUBLIC_API_CONTRACTS` assigns exactly one disposition per matrix runtime API/member:

| Disposition | Meaning |
|-------------|---------|
| `e2e-outcome` | Example gallery screen with a deterministic success assertion (`contractId`, example component name, `gma.*` screen and assertion testIDs, success kind, assertion text). Must reference a known executable case in `tooling/appium/src/formats.ts`. |
| `lower-layer-only` | Jest or another gallery contract owns the observable outcome; this symbol is exercised only as plumbing. |
| `not-appium-capable` | Explicit maintainer reason — no vague “hard to test” wording. |
| `absent-from-example` | Public v17 surface with no example screen yet. |

`validatePublicApiContracts(matrix, contracts, App.tsx)` cross-checks compiler classification, registry completeness, `formats.ts` linkage, and that each `e2e-outcome` example component body contains the declared testID expressions. Device-free tests in `tooling/appium/__tests__/publicApiMatrix.test.ts` fail on missing/stale/duplicate dispositions, classification drift, or presence-only success assertions. A new runtime export fails until a maintainer adds its disposition; removing an export fails until the stale row is deleted.

**Consent/UMP parked.** Every runtime `AdsConsent*` export (namespace, methods, and related presets) is `not-appium-capable` with an explicit **parked** maintainer decision: the example intentionally omits a maintained UMP app ID and consent form, so Appium must not claim a consent outcome. Navigation smoke may still open the consent gallery row for container reachability; that row is not an API outcome contract. Native consent branches stay out of Jacoco expectations until a configured UMP test app exists (`DEVICE_NATIVE_COVERAGE_GAPS` id `consent-ump` in `nativeCoverageCheckpoints.ts`).

**Case inventory and derived shards.** Executable device cases are declared in `tooling/appium/src/formats.ts` (bounded-retry request-outcome contracts, navigation smoke, SDK utility surfaces, NativeRNGMATesting probe). `tooling/appium/src/sessionShards.ts` orders them into `SMOKE_CASES` (request-outcome cases first so they stay in the first positional shard), then `deriveSmokeShards()` packs them into whole waves of three positional sessions (`a-primary`, `b-secondary`, `c-tertiary`) under the fifteen-test session cap — growth adds waves (`a-primary-w2`, …), never a longer session. Every `e2e-outcome` `contractId` must appear in `SMOKE_CASES` (locked in `tooling/appium/__tests__/sessionShards.test.ts`). One WDIO spec exists per derived shard id (`tooling/appium/test/specs/formats.smoke.<shard-id>.spec.ts`); `yarn tests:appium:validate` imports each spec against the derived shard list.

**Native coverage checkpoints (`tooling/appium/src/nativeCoverageCheckpoints.ts`).** Behavioral proof stays in Appium assertions; this registry maps each executable Appium `contractId` to repo-relative Android/iOS product sources maintainers should expect in Jacoco/LCOV after teardown flush, plus a specific `jacocoExpectation` string for pull/report diffs ([coverage design § native agent collection](coverage-design.md#native-agent-collection)). `validateNativeCoverageCheckpoints()` keeps checkpoint count aligned with executable contracts and requires a checkpoint for every `e2e-outcome` `contractId`. `DEVICE_NATIVE_COVERAGE_GAPS` records honest remaining gaps (for example GAM fluid navigation-only, initialization UI not rendered, parked consent, iOS LCOV exporter limits, Android before/after baseline workflow). WDIO may log checkpoint counts during flush; agents still pull/report/assert per coverage design.
...
**Accepted outcome (settled).** Every representative session runs at most **ten fresh request attempts** and **stops immediately on the first accepted outcome** — remaining attempts are never taken. Every format on both platforms accepts only `loaded`. Every other terminal outcome — `no-fill`, any `internal-error` regardless of fingerprint, and `other-error` — is a **retry**. Retry backoff before attempts two and up is 250 ms, 500 ms, 1000 ms, then capped at 2000 ms. A session that reaches attempt ten without an accepted outcome **hard-fails** the format with a deterministic diagnostic carrying every attempt's `attempt`, `requestId`, `classification`, and `fingerprint`. A missing outcome marker, a timeout or other non-terminal wait, a missing, reused, or non-increasing monotonic `requestId`, or any other WebDriver / navigation / container / probe / seam failure **hard-fails immediately** — those never consume a retry.
...
| Native | android | 185 | 1 | 0 | 142 (+42 unfingerprinted) | 0 | `loaded` (+ render proof) |
...
The historical Native internal-error sample was collected on Google APIs images that cannot validate genuine Native fill/render. The fingerprint remains useful diagnostic evidence but is no longer accepted. API 36 `google_apis_playstore` loaded and rendered the official Native test creative, establishing the required emulator contract. The official GAM interstitial test unit has separately returned `no-fill` twice in a session where the standard Interstitial loaded.

**Sufficiency, and no top-up.** The 287 collect-mode attempts that preceded this snapshot were sufficient to settle acceptance; no further collection run is required. Every format/platform pair has produced the outcome its rule requires, including an Android Native loaded/rendered result on a Play Store-enabled image, so each rule is known to be satisfiable on a compatible device. That is a harness decision about which contract the suite can hold — not a statistical claim about the population of ad-server responses, and not a prediction of per-attempt fill rates. Qualifying proof runs after that decision only raise the lower-bound counts (299 here); they do not reopen collection.
...
| Appium config / drivers / testIDs-only (`tooling/appium/**` drivers/validate/helpers, [API-wide registries](running-e2e.md#api-wide-e2e-coverage) only, and/or example `testID` props only; no format gallery behavior change; no device specs change) | `yarn tests:appium:validate`. **Not** device Appium. **Supersedes** the example Appium device row for those edits. |
...
| Native unit-test harness only (Android [dual layout and commands](agent-command-policy.md#canonical-registry) and/or `packages/core/ios/Tests/**`, plus Robolectric/XCTest wiring in `packages/core/android/build.gradle`, podspec `test_spec` / `exclude_files`, or `packages/core/ios/RNGoogleMobileAdsUnitTests.xcodeproj`; no production native behavior change) | Touched platforms/backends: the matching canonical Android JVM command(s) and/or `yarn tests:ios:unit`. **Not** native e2e. **Supersedes** the plugin/native row for those harness-only edits. Does **not** apply when the Diff also touches production `packages/core/ios/RNGoogleMobileAds/**` / `packages/core/android/src/main/**` (e.g. OwnedMappers extract + `Common` delegate) — that stays on the plugin/native Appium row for the affected platform. |
| Next-Gen Android backend implementation or selection | Run the full Android smoke suite once with `RNGMA_ANDROID_BACKEND=nextgen` and once with the default classic backend. The serial Next-Gen command is `yarn tests:appium:android:nextgen`; the parallel command is `RNGMA_ANDROID_BACKEND=nextgen yarn tests:appium:android:parallel`. The ordinary classic commands remain `yarn tests:appium:android` / `yarn tests:appium:android:parallel`. This row **supersedes** the Android half of the plugin/native row; apply independently matching iOS rows. |
...
2. **Worker/session — 60 seconds** by default (`WORKER_STARTUP_TIMEOUT_MS`): every Appium child must emit both WDIO worker-start evidence (`Execution of … workers started`) and evidence that its WebDriver/Appium session was created. **iOS parallel only** (`yarn tests:appium:ios:parallel` / `ios:parallel:external`, and `ios:` children under combined `yarn tests:appium:parallel`): the per-child worker/session ceiling is **180 seconds** (`IOS_PARALLEL_WORKER_STARTUP_TIMEOUT_MS`) because concurrent XCUITest session create under three-sim contention can exceed 60s. Android children — including in combined parallel — remain on the **60 second** ceiling; the 180s allowance must never be applied globally to Android.
