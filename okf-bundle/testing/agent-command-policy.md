...
| Android JVM unit (Robolectric) | `yarn tests:android:unit` → example `./gradlew :react-native-google-mobile-ads:testDebugUnitTest`; runs neutral `packages/core/android/src/test/` plus classic `src/testClassic/` | ad-hoc `./gradlew … testDebugUnitTest` outside this yarn script; treating library-folder standalone AGP as the gate |
| Android Next-Gen JVM unit | `yarn tests:android:unit:nextgen` → the same example-hosted task with `-PRNGMA_ANDROID_BACKEND=nextgen`; runs neutral `src/test/` plus `src/testNextgen/` | direct Gradle invocation; treating the intentional production stub as a real Next-Gen implementation |
...
| Android example run | `yarn tests:android:run` — serial uses the example RN CLI `android` script; slot mode validates/boots the exact AVD+serial, proves selected-serial reverse/connectivity, then CLI 20.2 `run-android --device <exact serial> --binary-path <slot APK> --no-packager --port <worktree Metro>` only — [e2e slots](running-e2e.md#e2e-slots) | omitting `--device`; all-device install; invoking RN CLI outside this yarn script; extra Metro; invented `adb install` / `am start`; Gradle `installDebug`; installing onto an unrelated emulator |
| Android Next-Gen placeholder | `yarn tests:android:build:nextgen` builds the opt-in APK; `yarn tests:appium:android:nextgen-smoke` runs only the named intentional fail-fast spec on a selected Android slot | running the full Appium suite against the placeholder; treating the intentional error as a real backend |
