...
| Appium | `yarn tests:appium:validate`; after drivers: `yarn tests:appium:drivers:install` then `yarn tests:appium:drivers:verify`; device runs: `yarn tests:appium:android` / `yarn tests:appium:ios`; iOS simulator selection and prebuilt WDA: `yarn tests:appium:ios:select-and-boot --github-env <path>` (flag always required; local path may be any writable file) then `yarn tests:appium:ios:prebuild-wda` and `RNGMA_WDA_PREBUILT=1 yarn tests:appium:ios`, consuming the emitted `RNGMA_IOS_UDID` / `RNGMA_IOS_VERSION` per [§ prebuilt validation](running-e2e.md#ios-wda-prebuilt-validation) ([Appium](running-e2e.md#appium-scaffold)) | inventing `npx appium` / `npx wdio` outside those scripts; treating device Appium as required when [platform coverage](running-e2e.md#platform-coverage-gate-blocking) says validate-only; requiring `yarn tests:appium:ios:prebuild-wda` on every local Appium run; running `select-and-boot` without `--github-env`, or assuming a local `$GITHUB_ENV` exists |
