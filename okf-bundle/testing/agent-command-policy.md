...
Never detox/npm install/ad-hoc gradlew (except root `./gradlew ktlintCheck` / `./gradlew ktlintFormat`)/`npx google-java-format` (Java gate is yarn lint:android; Kotlin check is root `./gradlew ktlintCheck`, format is root `./gradlew ktlintFormat`). Do not invent `jet` / `npx jet` (Jet harness removed).
