...
| Android JVM unit (Robolectric) | `yarn tests:android:unit` → example `./gradlew :react-native-google-mobile-ads:testDebugUnitTest` (sources under `packages/core/android/src/test/`) | ad-hoc `./gradlew … testDebugUnitTest` outside this yarn script; treating library-folder standalone AGP as the gate |
| iOS XCTest unit | `yarn tests:ios:unit` → `xcodebuild test` on `packages/core/ios/RNGoogleMobileAdsUnitTests.xcodeproj` (sources under `packages/core/ios/Tests/`; podspec `UnitTests` test_spec mirrors them) | ad-hoc `xcodebuild test` outside this yarn script |
