...
| Native e2e Codegen | `yarn tests:e2e:codegen` (example RN CLI, `react-native codegen --platform all`; then removes only transient `RNGoogleMobileAdsExample/android/app/build/generated/source/codegen`; also chained before canonical native build/install and invoked by Appium preflight) | platform-specific Codegen; building/installing before Codegen; leaving that Android app codegen tree; copying the cleanup one-liner |
