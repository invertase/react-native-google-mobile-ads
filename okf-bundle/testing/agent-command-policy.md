...
| Native e2e Codegen | `yarn tests:e2e:codegen` (example RN CLI, `react-native codegen --platform all`; then removes only transient `RNGoogleMobileAdsExample/android/app/build/generated/source/codegen`; also chained into Android build/run and iOS run, while iOS pod-install remains the exact frozen bundled pod command; invoked by Appium preflight) | platform-specific Codegen; building/installing before Codegen; leaving that Android app codegen tree; copying the cleanup one-liner |
| Appium resource check/release | `yarn tests:appium:check` / `yarn tests:appium:release` with documented `--slot`, `--platform`, `--all-slots`, `--services`/`--strict`, `--devices`, and `--only` flags — [e2e slots](running-e2e.md#e2e-slots) | ad-hoc `lsof`/kill cleanup; slots `0`/`3`; unrelated listeners/devices; global simulator shutdown/delete/erase/rename; Detox clones |
...
| Appium | `yarn tests:appium:validate`; after drivers: `yarn tests:appium:drivers:install` then `yarn tests:appium:drivers:verify`; serial/manual device runs: `yarn tests:appium:android` / `yarn tests:appium:ios`; local three-slot owners: `yarn tests:appium:android:parallel` / `yarn tests:appium:ios:parallel` after exact create-only slot `1`/`2`/`4` provisioning — [parallel Appium](running-e2e.md#parallel-appium). Slot environment and manual sequences: [e2e slots](running-e2e.md#e2e-slots); every standalone iOS device run is prebuilt-WDA mode (`yarn tests:appium:ios` sets `RNGMA_WDA_PREBUILT=1` itself, no opt-out), so it needs `yarn tests:appium:ios:select-and-boot --github-env <path>` (flag always required; local path may be any writable file) then `yarn tests:appium:ios:prebuild-wda` first, consuming the emitted `RNGMA_IOS_UDID` / `RNGMA_IOS_VERSION` per [§ prebuilt WDA](running-e2e.md#ios-wda-prebuilt-validation) ([Appium](running-e2e.md#appium-scaffold)) | inventing `npx appium` / `npx wdio` outside those scripts; setting internal parallel spec/parent variables or invoking its workspace implementation directly; treating device Appium as required when [platform coverage](running-e2e.md#platform-coverage-gate-blocking) says validate-only; skipping provisioning when an exact slot device is missing; expecting Appium/select-and-boot to create; skipping the prebuild before standalone `yarn tests:appium:ios`, or requiring it for `yarn tests:appium:android` / `yarn tests:appium:validate`; running `select-and-boot` without `--github-env`, or assuming a local `$GITHUB_ENV` exists |
...
Native e2e Codegen: `yarn tests:e2e:codegen` only (`--platform all` then drop only the transient Android app codegen tree; already chained into canonical Android build/run and iOS run scripts and invoked by Appium preflight — never build or install first). iOS pods: root `BUNDLE_FROZEN=true bundle install` first, then `yarn tests:ios:pod:install` only (root `bundle exec` with `--project-directory`; root Gemfile). Android run: `yarn tests:android:run` only (serial RN CLI vs slot serial-scoped path: running-e2e.md#e2e-slots). iOS run: `yarn tests:ios:run --udid <selected-udid>` only (`build-ios --buildFolder build`, then exact-path `simctl install` / `launch`; slot-mode selector UDID/runtime: running-e2e.md#e2e-slots; artifact path on running-e2e.md#appium-scaffold).
E2e names: running-e2e.md#local-e2e-commands (tee of those same named scripts OK). Resource ownership/recovery and slot provisioning: running-e2e.md#e2e-slots (`yarn tests:appium:check` / `release` only for host probes and scoped cleanup; never slots 0/3 or unrelated resources; `RNGMA_E2E_SLOT=<1|2|4-7> yarn tests:appium:provision <android|ios|both>` create-only before first device use when missing; single-platform must match RNGMA_E2E_PLATFORM if set, both requires it unset; Appium/select-and-boot never create). Local parallel owners: running-e2e.md#parallel-appium (`yarn tests:appium:android:parallel` / `yarn tests:appium:ios:parallel` only, exact slots 1/2/4 must be provisioned and task-owned; never set internal spec/parent variables). Which to run: running-e2e.md#platform-coverage-gate-blocking. Do not glob yarn tests:android:* (Windows/release names) or yarn tests:ios:*. Do not run every named e2e script unless that table requires it. Jest, Android/iOS JVM/XCTest unit (`yarn tests:android:unit` / `yarn tests:ios:unit`), and packager stay Canonical registry names. Appium: running-e2e.md#appium-scaffold + registry Appium row (`yarn tests:appium:validate` / drivers / check / release / provision / android / ios / android:parallel / ios:parallel / ios:select-and-boot / ios:prebuild-wda only; `select-and-boot` always needs `--github-env <path>` and only writes that file; each standalone iOS device run is prebuilt mode (`tests:appium:ios` sets RNGMA_WDA_PREBUILT=1 itself), so select-and-boot then prebuild-wda first; sequence running-e2e.md#ios-wda-prebuilt-validation). Native coverage pull/report: coverage-design.md#native-agent-collection + registry Native coverage row (`yarn workspace RNGoogleMobileAdsExample exec rn-coverage …` only; no copied RNFB scripts).
