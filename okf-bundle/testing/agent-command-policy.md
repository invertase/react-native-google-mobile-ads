...
| Appium | `yarn tests:appium:validate`; after drivers: `yarn tests:appium:drivers:install` then `yarn tests:appium:drivers:verify`; device runs: `yarn tests:appium:android` / `yarn tests:appium:ios`; slot environment and exact sequences: [e2e slots](running-e2e.md#e2e-slots); every iOS device run is prebuilt-WDA mode (`yarn tests:appium:ios` sets `RNGMA_WDA_PREBUILT=1` itself, no opt-out), so it needs `yarn tests:appium:ios:select-and-boot --github-env <path>` (flag always required; local path may be any writable file) then `yarn tests:appium:ios:prebuild-wda` first, consuming the emitted `RNGMA_IOS_UDID` / `RNGMA_IOS_VERSION` per [§ prebuilt WDA](running-e2e.md#ios-wda-prebuilt-validation) ([Appium](running-e2e.md#appium-scaffold)) | inventing `npx appium` / `npx wdio` outside those scripts; treating device Appium as required when [platform coverage](running-e2e.md#platform-coverage-gate-blocking) says validate-only; skipping provisioning when an exact slot device is missing; expecting Appium/select-and-boot to create; skipping the prebuild before `yarn tests:appium:ios`, or requiring it for `yarn tests:appium:android` / `yarn tests:appium:validate`; running `select-and-boot` without `--github-env`, or assuming a local `$GITHUB_ENV` exists |
...
E2e names: running-e2e.md#local-e2e-commands (tee of those same named scripts OK). Slot provisioning and exact slot sequences: running-e2e.md#e2e-slots (`RNGMA_E2E_SLOT=<1|2|4-7> yarn tests:appium:provision <android|ios|both>` create-only before first device use when missing; single-platform must match RNGMA_E2E_PLATFORM if set, both requires it unset; Appium/select-and-boot never create). Which to run: running-e2e.md#platform-coverage-gate-blocking. Do not glob yarn tests:android:* (Windows/release names) or yarn tests:ios:*. Do not run every named e2e script unless that table requires it. Jest, Android/iOS JVM/XCTest unit (`yarn tests:android:unit` / `yarn tests:ios:unit`), and packager stay Canonical registry names. Appium: running-e2e.md#appium-scaffold + registry Appium row (`yarn tests:appium:validate` / drivers / provision / android / ios / ios:select-and-boot / ios:prebuild-wda only; `select-and-boot` always needs `--github-env <path>` and only writes that file; every iOS device run is prebuilt mode (`tests:appium:ios` sets RNGMA_WDA_PREBUILT=1 itself), so select-and-boot then prebuild-wda first; sequence running-e2e.md#ios-wda-prebuilt-validation). Native coverage pull/report: coverage-design.md#native-agent-collection + registry Native coverage row (`yarn workspace RNGoogleMobileAdsExample exec rn-coverage …` only; no copied RNFB scripts).
