...
| iOS example run | `yarn tests:ios:run` (`react-native run-ios --buildFolder build`; exact app path: [running-e2e](running-e2e.md#appium-scaffold)) | `run-ios` without `--buildFolder build`; discovering an app from DerivedData |
...
Native e2e Codegen: `yarn tests:e2e:codegen` only (`--platform all` then drop only the transient Android app codegen tree; already chained into canonical Android/iOS build/install scripts and invoked by Appium preflight — never build or install first). iOS pods: root `BUNDLE_FROZEN=true bundle install` first, then `yarn tests:ios:pod:install` only (root `bundle exec` with `--project-directory`; root Gemfile). iOS run: `yarn tests:ios:run` only (`--buildFolder build`; artifact path on running-e2e.md#appium-scaffold).
