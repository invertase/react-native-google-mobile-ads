...
| android JVM unit | Matching classic/Next-Gen command(s) from the [canonical registry](agent-command-policy.md#canonical-registry) | 0 | if a neutral/backend-specific Android test layout or Robolectric wiring changed — [platform coverage harness-only](running-e2e.md#platform-coverage-gate-blocking) |
