...
<Accordion title="app.json" defaultOpen>
```json
...
<Accordion title="app.config.ts">
```ts
// <project-root>/app.config.ts
export default {
  plugins: [
    [
      'expo-build-properties',
  ...
  plugins: [
    [
      'react-native-google-mobile-ads',
      {
        ...
        iosAppId: 'ca-app-pub-xxxxxxxx~xxxxxxxx',
        skAdNetworkItems: [
          // SKAdNetwork identifiers as shown in the app.json example above
        ]
      }
    ]
  ]
