...
> Attempting to build your app without a valid App ID in `app.json` or `app.config.js / app.config.ts` will cause the app to crash on start or fail to build.
...
Within the root of your React Native project, open the `app.json` or (if using Expo) `app.config[.js|.ts]` file and add the
...
If you're an expo user, make sure the `react-native-google-mobile-ads` block is in the expo.extra
