...
```bash
# Install the admob module (COMING SOON)
# yarn add @invertase/react-native-google-ads

# For the time being (as there is no NPM package available yet)
# add the following entry to your package.json and run yarn
"react-native-google-ads": "github:invertase/react-native-google-ads"
```
...
```js
import admob, { MaxAdContentRating } from 'react-native-google-ads';
...
```jsx
import { InterstitialAd, RewardedAd, BannerAd, TestIds } from 'react-native-google-ads';
