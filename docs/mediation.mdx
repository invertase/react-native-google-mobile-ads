...
For your cross-platform code to work seamlessly, the name of the module must be identical on both iOS and Android.

- On Android, the name is determined by the `String` you return in the `getName()` method.

- On iOS, the name is determined by the class name itself when you use the `RCT_EXPORT_MODULE()` macro.
...
// This exports the module with the name "AdMobMediation" by default.
RCT_EXPORT_MODULE();
