...
so only Next-Gen is on the classpath. AdMob-hosted mediation is supported; **non-AdMob
mediation hosts** (for example AppLovin MAX) cannot select Next-Gen — stay on classic
Android for those apps. Google Ad Manager unit IDs remain supported under Next-Gen.
