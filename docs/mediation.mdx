      ...
      [
        "expo-build-properties",
        {
          ...
          "ios": {
            "pods": [
              {
                // Example: Add the AppLovin mediation adapter for iOS. Replace {X.Y.Z.A} with the specific adapter version you are integrating.
...
On iOS, create a header (`.h`) and an implementation (`.m`) file. The following example creates an `GoogleMobileAdsMediationAppLovin` class:
...
Create an [Expo module](https://docs.expo.dev/modules/get-started/) with the `create-expo-module` tool.
...
When prompted, give your module a name. This will scaffold a new module in a modules/ directory at your project root. The following example creates a module to use with the
...
dependencies {
  ...
  implementation 'com.applovin:applovin-sdk:{X.Y.Z.A}'
}
```
...
```
# Add the third-party pod dependency. Replace {X.Y.Z.A} with the specific adapter version you are integrating.
s.dependency 'AppLovinSDK', '~> {X.Y.Z.A}'
```

Install and rebuild.

After creating the module and adding the code, install it in your project and rebuild your development client.
