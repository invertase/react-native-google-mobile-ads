...
The Google's UMP (Universal Messaging Platform) SDK is updated to support the latest IAB standards, and greatly 
simplifies the process of setting up consent forms and ad partner listings.
...
> We recommend including legitimate interest in your GDPR-compliant messages. Well, if you do not include it, your income could be significantly reduced
...
const requestAdsConsent = async () => {
  ...
  if (
    ...
    consentInfo.status = await AdsConsent.showForm();
  }
...
Then, after displaying the consent forms using UMP, you can start requesting ads. If you have set `requestNonPersonalizedAdsOnly` (also called `NPA`) in your ad request, [the AdMob SDK will look at that and the consent indicated by the `TC String` and apply the most conservative setting](https://support.google.com/admob/answer/9760862). In other words, AdMob will automatically handle the request with the consent status selected by the user.

```js
const interstitial = InterstitialAd.createForAdRequest(
  "ca-app-pub-3940256099942544/4411468910", // unit id for testing; remember to change...
  {
...
In the example above, we have indicated that we want to display personalized ads to the user (`NPA = 0`). In the event that said user does not give us consent for personalized ads through the GDPR form, we could modify the `RequestOptions` object (by setting `requestNonPersonalizedAdsOnly` to `true` (that is, `NPA = 1`)) or simply let AdMob automatically decide for us, using the most restrictive option (in this case, based on the user choices, it will not serve personalized ads).

> If the user also does not agree to the basic ads consent, AdMob will use the [limited ads](https://support.google.com/admob/answer/10105530?hl=en) feature, which gives us the ability to serve ads in a limited way in the absence of consent for the use of cookies or other local storage, as referenced Google's EU User Consent policy. This may decrease the frequency of ads a bit, but it's better than not showing any ads at all.
