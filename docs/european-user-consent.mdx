...
description: Gather and manage consent with Google's User Messaging Platform through React Native.
---
...
<Info>
  The `AdsConsent` helper only provides you with the tools for requesting consent, it is up to the
  developer to ensure the consent status is reflected throughout the app.
</Info>
...
If your app will be used by users within the EEA, it is important you delay app measurement until you initialize the Google Mobile Ads SDK or load an ad (after consent).
...
This will also show an IDFA explainer message. If you don't want to show an explainer message you can show the ATT alert [manually](/consent-basics#requesting-app-tracking-transparency-authorization-ios).
...
the actual user consent**. It simply indicates if you now have a consent response to decode.
...
`getUserChoices` / `getTCModel` decode the IAB TCF string with a transitive `@iabtcf/core` dependency. Apps that only use this module do **not** need to add `@iabtcf/core` themselves. If Metro reports `UnableToResolveError` for `./gvl/index.js` under `@iabtcf/core`, confirm `node_modules/@iabtcf/core/lib/mjs/model/gvl/index.js` exists on disk, reinstall dependencies, and restart Metro with a reset cache — a missing file is an incomplete install, not a missing export from this package.
