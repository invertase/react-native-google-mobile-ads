...
The `AdsConsent` helper provides a promised based method to return the users consent status called `requestInfoUpdate`.
...
The result of the method returns an `AdsConsentInfo` interface, which provides information about consent form availability and the users consent status:
...
If using a real device, ensure you whitelist it by passing the device ID, which can be obtained from native logs or using a library
...
It is also possible to reset the UMP state for test devices. This does not reset the ATT state though.
