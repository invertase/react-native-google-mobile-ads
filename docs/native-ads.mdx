...
Request validation failures reject with a standard `Error`. Native load failures reject with the
`AdError` type. For compatibility, both platforms use `code: 'ERROR_LOAD'`; `message` contains the
underlying SDK / platform failure text. Use `reason` for detailed error handling.
