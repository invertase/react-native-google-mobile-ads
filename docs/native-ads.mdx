...
Rejected errors match the `NativeAdLoadError` interface. Both platforms use `code: 'ERROR_LOAD'`;
`message` contains the underlying SDK / platform failure text.
See [NativeAdLoadError](https://github.com/invertase/react-native-google-mobile-ads/blob/main/src/types/NativeAdLoadError.ts) for the type definition.
...
NativeAd.createForAdRequest(TestIds.NATIVE)
  ...
  .catch((error) => {
    ...
    // error.code === 'ERROR_LOAD'
    // error.message describes the failure (no fill, invalid request, …)
  });
```
