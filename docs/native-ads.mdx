...
Rejected errors match the `NativeAdLoadError` interface (`code` and `message`).
Common codes include `error-code-invalid-request`, `error-code-no-fill`, and `ERROR_LOAD` when the loaded ad has no valid response ID.
