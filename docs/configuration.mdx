...
- **Expo:** config plugin (`androidAppId` / `iosAppId`) — see [Installation — Expo](/installation/expo).
- **Bare React Native:** root `app.json` snake_case (`android_app_id` / `ios_app_id`) — see
  [Installation — React Native CLI](/installation/react-native). Missing bare keys produce native
