...
`.catch()` (or `try { await } catch`) covers them. A destroyed or misused ad still throws
synchronously, on purpose, as a loud programmer-error signal. If you drive fullscreen ads through
...
The error messages are the same as before. If you wrapped `show()` in a synchronous `try` /
...
// A destroyed instance or invalid options still throw synchronously (programmer error) — fix the
// call site.
```
