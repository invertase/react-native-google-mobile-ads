...
description: Adopt the v17 platform baseline, structured outcomes, pools, and multi-format APIs.
---
...
[generated API reference](https://invertase.github.io/react-native-google-mobile-ads/).
...
7. On v17.2 or newer, handle not-loaded and already-showing `show()` outcomes with `.catch()` —
   see
   [v17.2: `show()` reports not-loaded and already-showing on the promise](#v172-show-reports-not-loaded-and-already-showing-on-the-promise).

## v17.2: `show()` reports not-loaded and already-showing on the promise

We recognize v17 is very new and expect essentially no one has adopted the new v17 APIs yet. This
smooths a real rough edge: the two most common "not right now" conditions on `show()` — not loaded,
and already showing — now come back on the returned promise, like a platform decline, so a single
`.catch()` (or `try { await } catch`) covers them. A destroyed ad, or invalid show options on a
loaded ad, still throw synchronously, on purpose, as a loud programmer-error signal. If you drive fullscreen ads through
the fullscreen hooks, nothing changes — the hook `show` was always press-safe. Pooled ads have a
small delta of their own, described at the end of this section.

What changed for `MobileAd.show()` (`InterstitialAd`, `RewardedAd`, `RewardedInterstitialAd`,
`AppOpenAd`, `GAMInterstitialAd`):

| Condition                          | v17.1 and earlier    | v17.2 and newer                  |
| ---------------------------------- | -------------------- | -------------------------------- |
| Not loaded yet                     | Throws synchronously | Rejects the returned promise     |
| Show already requested (in flight) | Throws synchronously | Rejects the returned promise     |
| Platform declines to present       | Rejects              | Rejects (unchanged)              |
| Ad has been destroyed              | Throws synchronously | Throws synchronously (unchanged) |
| Structurally invalid `showOptions` | Throws synchronously | Throws synchronously (unchanged) |

A destroyed ad always throws. Otherwise, the not-loaded and in-flight checks reject first, so
invalid `showOptions` throw only on a loaded ad with no show in flight. The error messages are the
same as before. If you wrapped `show()` in a synchronous `try` /
`catch` to handle not-loaded or already-showing, move that handling to the promise:

```ts
// v17.1 and earlier: not-loaded and already-requested were thrown synchronously.
try {
  interstitial.show();
} catch (error) {
  console.warn(error);
...
// v17.2 and newer: not-loaded, already-showing, and platform declines come back on the promise.
interstitial.show().catch(error => {
  console.warn(error);
});
// A destroyed instance, or invalid options on a loaded ad, still throw synchronously (programmer
// error) — fix the call site.
```

The existing fullscreen hooks (`useInterstitialAd`, `useRewardedAd`, `useRewardedInterstitialAd`,
`useAppOpenAd`) are unchanged: their `show` absorbs both channels, so an `onPress={() => show()}`
is a no-op on failure and hook call sites need no change. The new `useGAMInterstitialAd` shares
the same press-safe `show`.

Preload pools have been public since the first v17 release, so pooled fullscreen ads get a small
delta of their own. A
destroyed `PooledAd` now **throws synchronously** instead of rejecting, matching `MobileAd` and
the rest of the API's "using a destroyed object is a programmer error" convention. If you were
relying on `pooledAd.show()` rejecting after the ad was destroyed, move that handling to a
synchronous `try` / `catch` (or simply don't call `show()` on a destroyed or consumed pooled ad).

The ad returned by `usePooledAd` moves a little further. In v17.1 and earlier its `show()` was
an `async` wrapper, so every failure, including a destroyed ad and invalid options, came back as
a rejection. It now keeps the same two channels as `PooledAd.show()`. Neither is press-safe like
the fullscreen hooks' `show`.

| Condition                                | v17.1 and earlier `PooledAd` | v17.1 and earlier `usePooledAd` ad | v17.2 and newer (both)                          |
| ---------------------------------------- | ---------------------------- | ---------------------------------- | ----------------------------------------------- |
| Show already requested (in flight)       | Rejects                      | Rejects                            | Rejects (unchanged)                             |
| Platform declines to present             | Rejects                      | Rejects                            | Rejects (unchanged)                             |
| Ad has been destroyed                    | Rejects                      | Rejects                            | Throws synchronously                            |
| Invalid `showOptions`, no show in flight | Throws synchronously         | Rejects                            | Throws synchronously (unchanged for `PooledAd`) |

A pooled ad is always loaded when you poll it, so there is no not-loaded case.
...
asks native to drop the holder. Destroyed instances ignore late events and `load()`; `show()` on a
destroyed instance throws synchronously.
...
expose load cancellation). See [React hooks](/ad-formats/hooks).
...
| Types, expiry, ownership            | [Generated `AdPool` reference](https://invertase.github.io/react-native-google-mobile-ads/AdPool.html) |
...
| Post-show events never fire after pooled show          | Hook-owned show destroys the ad on `CLOSED` — `release()` before show if you need events after close |
...
“no ads” reports. See
[Common reasons ads do not show](/common-reasons-for-ads-not-showing) and
[Ad Inspector](/ad-inspector).

| Symptom                                                 | Likely cause / fix                                                                                                                              |
| ------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| `googleMobileAds/timeout` ~ten seconds after initialize | SDK `GADErrorTimeout` forwarding — not a bridge timer; see [common reasons](/common-reasons-for-ads-not-showing#googlemobileadstimeout-on-load) |
...
- [React hooks](/ad-formats/hooks)
- [Mediation](/mediation)
- [Common reasons ads do not show](/common-reasons-for-ads-not-showing)
- [Migrating to v15](/migrating-to-v15) — prior Android Kotlin floor note
- [Generated API reference](https://invertase.github.io/react-native-google-mobile-ads/) — public types and symbol-level guidance
