...
### Non-AdMob mediation hosts

Mediation hosts that are **not** AdMob (for example AppLovin MAX) **cannot** select
Next-Gen. Stay on classic Android for those apps.

### Google Ad Manager

Google Ad Manager is supported under Next-Gen. Ad Manager unit IDs (paths starting with
`/`, for example `/6499/example/native-advanced`) load through the same request builders
...
Prefer presets over hand-rolled capability matrices. See the
[v17 API reference](/rngma-v17-api-reference#capability-discovery-new) and
