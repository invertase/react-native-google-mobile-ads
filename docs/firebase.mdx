...
[Installation — Expo](/installation/expo) or [Installation — React Native CLI](/installation/react-native)
...
- **New Architecture is required.** Explicit Legacy Architecture opt-out fails CocoaPods install
  for this module on iOS. On React Native 0.82+, some Android hosts ignore `newArchEnabled=false`;
  treat New Architecture as mandatory on both platforms.
...
2. Install this module and set App IDs in `app.json` — see
   [Installation — React Native CLI](/installation/react-native).
...
RNFB native module you use in `forceStaticLinking` (required on current Expo / React Native
prebuilt core — see [RNFB iOS SPM Support](https://rnfirebase.io/ios-spm)):
...
2. Configure `use_frameworks! :linkage => :static` as required by your Firebase / other native
   ...
   [Installation — React Native CLI](/installation/react-native#optionally-configure-ios-static-frameworks)).
...
- Treating the install-page static-frameworks snippet as the React Native Firebase default (see
  [Installation — Expo](/installation/expo#optionally-configure-ios-static-frameworks) and
  [Installation — React Native CLI](/installation/react-native#optionally-configure-ios-static-frameworks);
