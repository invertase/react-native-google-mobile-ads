...
[generated API reference](https://invertase.github.io/react-native-google-mobile-ads/modules.html).
...
## Consent before pools

`AdPools.create` initializes the Google Mobile Ads SDK on Android and starts preloading
immediately. Create pools only after consent is resolved:

- **Declarative:** pass `enabled={consentReady}` to `AdPoolProvider`. `enabled` defaults to
  `true`. `false` stops **future** creates only; it does not destroy, cancel, or tear down
  pools that already exist.
- **Imperative:** call `AdPools.create` only after your consent flow has finished.

See [European User Consent](/european-user-consent).

## Recipe: fullscreen interstitial pool

Canonical path for warming a single fullscreen format. Same pattern applies to
...
function LevelEndButton() {
  ...
  const onPress = useCallback(async () => {
    ...
    const next = release();
    if (!next || next.format !== AdFormat.INTERSTITIAL) {
      next?.destroy();
      return;
    }
...
export function AppWithFullscreenPool({ consentReady }: { consentReady: boolean }) {
  const pools = useMemo(() => [fullscreenPool], []);
  return (
    <AdPoolProvider pools={pools} enabled={consentReady}>
      <LevelEndButton />
    </AdPoolProvider>
  );
}
...
- Pooled `show()`, on an imperative `PooledAd` or on the ad `usePooledAd` returns, has the
  same two error channels as `InterstitialAd.show()`. A show already in flight or a platform
  decline rejects the promise, so `.catch()` it. A destroyed ad throws synchronously, and so do
  invalid `showOptions` when no show is in flight. It is not press-safe like the fullscreen
  hooks' `show`, and `PooledAd` has no destroyed or loaded flag to check. Call `show()` only on
  the ad you just polled or released (or on the hook's `ad` while `status === 'filled'`), never
  after you have destroyed it, keep one clear owner, and `await` it inside `try` / `catch` or
  `.catch()` it.

Imperative equivalent (no React provider; run after consent is resolved):
...
if (result.status === 'filled' && result.ad.format === AdFormat.INTERSTITIAL) {
  const ad = result.ad;
  if (ad.isStaleByPolicy()) {
    ad.destroy();
  } else {
    // show() resolves once presentation starts, not when the ad closes.
    const offClosed = ad.addAdEventListener(AdEventType.CLOSED, () => {
      offClosed();
      ad.destroy();
    });
    try {
      // Awaiting inside try catches both the rejection and a synchronous throw.
      await ad.show();
    } catch (error) {
      offClosed();
      ad.destroy();
...
| `maxManagedPoolAds`         | Always `null` (server-delivered). Google's documented default app-wide cap is **6**                                                                                                                  |
...
`null`. Google documents a default limit of **6** preloaded ads, shared across all formats and
preload IDs, for [iOS](https://developers.google.com/admob/ios/ad-preloading#preload-cache-limits),
[Android](https://developers.google.com/admob/android/ad-preloading#preload-cache-limits), and
[Android Next-Gen](https://developers.google.com/admob/android/next-gen/ad-preloading#preload-cache-limits).
Shallow pools coexist comfortably;
...
- [React hooks](/ad-formats/hooks) — fullscreen, native, and pool hooks
