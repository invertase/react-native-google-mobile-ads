...
<Warning>
  Complete consent and SDK initialization before calling `NativeAd.createForAdRequest()`. See
...
```tsx
import { useEffect, useState } from 'react';
import { NativeAd, NativeAdEventType, NativeAdView, TestIds } from 'react-native-google-mobile-ads';
