...
description: Use React hooks to load and show app open, interstitial, rewarded, and rewarded interstitial ads.
---

# Fullscreen ads with hooks

The package provides hooks for all four fullscreen formats: app open, interstitial, rewarded,
and rewarded interstitial. The v17 options form loads automatically by default and exposes one
...
export default function RewardedInterstitialButton() {
  ...
  return (
    <Button
      title={earnedReward ? 'Reward earned' : 'Continue'}
...
unavailable on the Android classic backend; that pool limitation does not apply to this
load-on-demand hook.
