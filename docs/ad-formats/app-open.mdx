...
`AdEventType.LOADED`, `AdEventType.CLOSED`, and `AdEventType.ERROR`, record when loading finished,
and call `appOpenAd.show().catch(...)` only while the ad is less than four hours old. Not
loaded, already showing, and platform declines reject the returned promise; a destroyed ad or
invalid `showOptions` throw synchronously (a programmer error to fix at the call site). Create a new instance for each
...
Google's published app open guidance uses a four-hour freshness window. `useAppOpenAdManager`
enforces it for you. If you manage `AppOpenAd` yourself, record when loading completes and request a
replacement instead of showing older inventory. If you use preload pools, the handed-out object
