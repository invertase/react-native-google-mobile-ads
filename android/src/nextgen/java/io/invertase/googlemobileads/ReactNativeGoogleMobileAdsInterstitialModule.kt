...
class ReactNativeGoogleMobileAdsInterstitialModule(reactContext: ReactApplicationContext?) :
  ...
  override fun getAdEventName() = ReactNativeGoogleMobileAdsEvent.GOOGLE_MOBILE_ADS_EVENT_INTERSTITIAL
  ...
  @ReactMethod
  fun interstitialShow(requestId: Int, adUnitId: String, showOptions: ReadableMap, promise: Promise) {
    show(requestId, adUnitId, showOptions, promise)
  }

  override fun loadAd(adRequest: AdRequest, adLoadCallback: AdLoadCallback<InterstitialAd>) {
    InterstitialAd.load(adRequest, adLoadCallback)
  }

  override fun attachAdEventCallback(ad: InterstitialAd, callback: SharedAdEventCallback) {
    ad.adEventCallback = object : InterstitialAdEventCallback, AdEventCallback by callback {
  ...
  override fun showAd(ad: InterstitialAd, activity: Activity, onUserEarnedReward: (RewardItem) -> Unit) {
    ad.show(activity)
  }

  override fun setImmersiveModeOnAd(ad: InterstitialAd, enabled: Boolean) {
