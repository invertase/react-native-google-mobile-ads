...
- **Imperative fullscreen `ad.show()`:** Not loaded, already showing, or a platform decline reject the promise — `.catch(...)` it; a destroyed ad, or invalid `showOptions` on a loaded ad with no show in flight, throw synchronously (programmer error — fix the call site). Pooled fullscreen `show()` (a polled `PooledAd`, including the ad `usePooledAd` returns) follows the same two channels and is **not** press-safe like the fullscreen hooks' `show`
...
- **App open ads on cold-start loading screen + warm foreground:** `useAppOpenAdManager` (`showAdIfAvailable()`; 4-hour freshness). Strongest consent gate is `adUnitId: consentReady ? unitId : null` (or not mounting). `autoLoad: false` also blocks every request (preload, `showAdIfAvailable()`, warm foreground, post-close reload) but still shows an ad already held. On Android the manager does not auto-show right after another fullscreen ad from this library closes; a real home / app-switcher return still shows (iOS never backgrounds for these ads)
...
For options-form hooks (except `useAppOpenAdManager`): keep a real `adUnitId` once the placement exists; gate loading with `autoLoad: consentReady` (or call `load()` / `retry()` after consent). Putting consent only in `adUnitId` retires the placement when consent is false — that is a different policy. `useAppOpenAdManager` is the exception: prefer `adUnitId: null` (or do not mount it) until consent, which also drops any held ad. `autoLoad: false` blocks every request there too, but an ad already held can still show.
