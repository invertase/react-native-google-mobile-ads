...
/**
 * Regression for [#801](https://github.com/invertase/react-native-google-mobile-ads/issues/801):
 * re-delivering an identical FLUID sizeConfig must not mark propsChanged (that schedules
 * requestAd → destroy/reload), and fluid measure/layout must adopt measured height so Yoga
 * and the ad view stop fighting.
 */
...
class ReactNativeGoogleMobileAdsBannerAdFluidReloadTest {
  ...
  fun fluidMeasureAndLayout_adoptsMeasuredHeight() {
    ...
    // Invoke the same Runnable requestLayout posts (avoid ViewRoot re-layout fighting Robolectric).
