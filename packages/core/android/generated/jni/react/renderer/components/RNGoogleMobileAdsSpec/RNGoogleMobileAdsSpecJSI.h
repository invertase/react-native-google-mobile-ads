...
protected:
  NativeGoogleMobileAdsModuleCxxSpec(std::shared_ptr<CallInvoker> jsInvoker) : TurboModule(std::string{NativeGoogleMobileAdsModuleCxxSpec::kModuleName}, jsInvoker) {
    ...
    methodMap_["setAudioSessionIsApplicationManaged"] = MethodMetadata {.argCount = 1, .invoker = __setAudioSessionIsApplicationManaged};
...
private:
  ...
  static jsi::Value __setAudioSessionIsApplicationManaged(jsi::Runtime &rt, TurboModule &turboModule, const jsi::Value* args, size_t count) {
    static_assert(
      bridging::getParameterCount(&T::setAudioSessionIsApplicationManaged) == 2,
      "Expected setAudioSessionIsApplicationManaged(...) to have 2 parameters");
    bridging::callFromJs<void>(rt, &T::setAudioSessionIsApplicationManaged,  static_cast<NativeGoogleMobileAdsModuleCxxSpec*>(&turboModule)->jsInvoker_, static_cast<T*>(&turboModule),
      count <= 0 ? throw jsi::JSError(rt, "Expected argument in position 0 to be passed") : args[0].asBool());return jsi::Value::undefined();
  }
