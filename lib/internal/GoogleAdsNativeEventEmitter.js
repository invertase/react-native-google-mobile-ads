...
const { RNGAAppModule } = NativeModules;
...
class GoogleAdsNativeEventEmitter extends NativeEventEmitter {
  ...
  addListener(eventType, listener, context) {
    if (!this.ready) {
      RNGAAppModule.eventsNotifyReady(true);
      this.ready = true;
    }
    RNGAAppModule.eventsAddListener(eventType);

    let subscription = super.addListener(`rnga_${eventType}`, listener, context);
    ...
    subscription.eventType = `rnga_${eventType}`;
    ...
    let newRemove = () => {
      RNGAAppModule.eventsRemoveListener(eventType, false);
  ...
  removeAllListeners(eventType) {
    RNGAAppModule.eventsRemoveListener(eventType, true);
    super.removeAllListeners(`rnga_${eventType}`);
  }

  // This is likely no longer ever called, but it is here for backwards compatibility with RN <= 0.64
  removeSubscription(subscription) {
    RNGAAppModule.eventsRemoveListener(subscription.eventType.replace('rnga_'), false);
