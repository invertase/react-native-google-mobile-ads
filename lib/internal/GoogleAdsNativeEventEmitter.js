...
const { RNGoogleApp } = NativeModules;
...
class GoogleAdsNativeEventEmitter extends NativeEventEmitter {
  ...
  addListener(eventType, listener, context) {
    if (!this.ready) {
      RNGoogleApp.eventsNotifyReady(true);
      this.ready = true;
    }
    RNGoogleApp.eventsAddListener(eventType);
    ...
    let newRemove = () => {
      RNGoogleApp.eventsRemoveListener(eventType, false);
  ...
  removeAllListeners(eventType) {
    RNGoogleApp.eventsRemoveListener(eventType, true);
    super.removeAllListeners(`rnapp_${eventType}`);
  }

  // This is likely no longer ever called, but it is here for backwards compatibility with RN <= 0.64
  removeSubscription(subscription) {
    RNGoogleApp.eventsRemoveListener(subscription.eventType.replace('rnapp_'), false);
